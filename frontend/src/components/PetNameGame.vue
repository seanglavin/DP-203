    ...
    methods: {
      async fetchGameBoards() {
        ...
        try {
          ...
          const endpoint = `/api/petfinder/pets/gameboards${queryString ? '?' + queryString : ''}`;
      ...
      loadCurrentPet() {
        ...
        this.currentPet = currentBoard[this.currentPetIndex];
      ...
      getAllPetNames() {
        ...
        this.gameBoards.forEach(board => {
          board.forEach(pet => {
            if (pet.name && pet.name !== this.currentPet.name) {
              names.push(pet.name);
            }
          });
        });
      ...
      nextPet() {
        ...
        if (this.currentPetIndex >= this.gameBoards[this.currentBoardIndex].length) {
          this.currentPetIndex = 0;
          this.currentBoardIndex++;
